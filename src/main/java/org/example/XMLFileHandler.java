...
public class XMLFileHandler {
    ...
    private Vehicle createVehicleFromElement(Element vehicleElement, String dealerId, String dealerName) {
        try {
            // Get vehicle type with explicit error handling
            String vehicleType = "";
            if (vehicleElement.hasAttribute("type")) {
                vehicleType = vehicleElement.getAttribute("type").toLowerCase();
            }

            // Create vehicle based on type with explicit logging
            Vehicle vehicle;
            switch (vehicleType) {
                case "suv":
                    vehicle = new SUV();
                ...
                case "sports car":
                    ...
                    break;
                default:
                    // Log a warning instead of silently converting
                    System.err.println("WARNING: Unrecognized vehicle type '" + vehicleType + "' for vehicle. Skipping vehicle import.");
